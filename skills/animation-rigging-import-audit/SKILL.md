---
name: animation-rigging-import-audit
description: Use when character animation needs an audit across skeleton, rig, skin weights, avatar, retargeting, clips, events, root motion, compression, import settings, runtime blending, and platform budgets; not for general art preflight.
version: 0.1.0
...
metadata:
  studio:
    ...
    side_effects: none
    artifact: animation-rigging-audit.json
...
## Overview
Provide an evidence-first contract for skeleton hierarchy, bind pose, skin weights, avatar, retargeting, clips, events, root motion, compression, blending, and budgets.
...
## When NOT to use
Do not use for unrelated texture, mesh, or general asset delivery review.
...
## Workflow
1. Bind source rig, export, engine importer, avatar, clips, and runtime controller.
   Completion criterion: evidence is recorded and unresolved items are explicit.
2. Compare skeleton hierarchy, bind pose, scale, orientation, weights, and constraints.
   Completion criterion: evidence is recorded and unresolved items are explicit.
3. Audit clip ranges, loop settings, events, root motion, masks, and retargeting.
   Completion criterion: evidence is recorded and unresolved items are explicit.
4. Check compression error, memory, CPU, blend artifacts, and platform limits.
   Completion criterion: evidence is recorded and unresolved items are explicit.
5. Return reproducible findings and safe re-export or import recommendations.
   Completion criterion: evidence is recorded and unresolved items are explicit.

## Evidence and output contract
Produce `animation-rigging-audit.json` with scope, snapshot, findings, owners, commands and exit codes, artifacts, acceptance criteria, Verified facts, Snapshot assumptions, Unverified hypotheses, BLOCKED items, and next actions.
