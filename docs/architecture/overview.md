...
The catalog contains 35 canonical skills: 15 in `studio-core`, 6 in `unity`, 9 in `cpp-lua-mmorpg`, and 8 in `production-design-liveops`. Shared skills may appear in more than one pack without duplicating canonical source text. `.codex-plugin/plugin.json` exposes that catalog directly. Hermes and Codex adapter exports contain 35 generated skills plus `registry.json`.
