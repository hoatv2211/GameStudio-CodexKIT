...
Adapter generation keeps one stable CLI seam in `scripts/generate_adapters.py`. Its implementation is split by lifecycle: `adapter_sources.py` owns validated source discovery and immutable capture, `standard_adapters.py` owns Hermes/Codex staging and atomic publication recovery, and `project_adapters.py` owns report, apply, uninstall, quarantine, and durable recovery for project-local installation. Keep transaction invariants inside their owning module and do not patch generated adapter outputs by hand.
