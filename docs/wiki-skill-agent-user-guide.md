...
> covers all **52 canonical skills**, **24 canonical agent roles**, and the
...
| `studio-goal-progress` | An opted-in KIT Goal needs verified percentage, ETA, recovery, or a read-only progress view | `$studio-goal-progress Open Progress for the active KIT-managed Goal.` |
| `studio-context-brief` | An active KIT Goal needs bounded brief, working, or resume context without replacing handoff | `$studio-context-brief Build the working projection without replacing studio-handoff.` |
...
`code-intelligence-contract`, `studio-goal-progress`, `studio-context-brief`,
`unity-ui-art-and-motion-production`, and
`game-screenshot-showcase-and-store-packaging` are currently `experimental`.
They require relevant provider, Goal runtime, Figma, Unity, capture, and reviewer
evidence before a runtime `PASS` can be claimed.
